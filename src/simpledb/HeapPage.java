...
public class HeapPage implements Page {
	...
	protected Tuple readTuple(byte[] data, int slotId) throws NoSuchElementException {
		...
		try {
			return readTuple(new DataInputStream(new ByteArrayInputStream(data, offset, data.length - offset)), slotId);
	...
	public byte[] getPageData() {
		...
		byte[] data = createEmptyPageData();
		writeInt(data, 0,tuples.size());
		Iterator<Tuple> t = tuples.iterator();
		int i=1;
		while(t.hasNext()){
			Tuple tup = (Tuple)t.next();
			byte[] r = toByteArray(tup);
		
			System.arraycopy(r, 0, data, data.length-(r.length*i), r.length);
			writeInt(data, i,i-1);
			i++;
		}
		return data;
